...
class Utils
{
    ...
    public static function request(string $variableName, mixed $defaultValue = null): mixed
    {
      ...
      /**
     * Retourne le prochain ordre de tri.
     * Si la colonne est déjà triée en ordre croissant, on passe en décroissant.
     * Sinon, on passe en croissant.
     * 
     ...
     * @return string
     */
    public static function getNextSortOrder(string $currentSort, string $currentOrder, string $column): string
    {
        if ($currentSort === $column && strtolower($currentOrder) === 'asc') {
            return 'desc';
        }
      ...
      /**
     * Retourne l'icône à afficher dans l'en-tête du tableau.
     * 
     * @param string $currentSort
     * @param string $currentOrder
     * @param string $column
     * @return string
     */
    public static function getSortArrow(string $currentSort, string $currentOrder, string $column): string
    {
        if ($currentSort !== $column) {
            return '↕';
        }

        return strtolower($currentOrder) === 'asc' ? '↑' : '↓';
    }

      /**
     * Génère l'URL de tri pour le tableau des articles.
     * 
     * @param string $column
     * @param string $articleSort
     * @param string $articleOrder
     * @param string $commentSort
     * @param string $commentOrder
     * @return string
     */
    public static function getArticleSortUrl(
        string $column,
        string $articleSort,
        string $articleOrder,
        string $commentSort,
        ...
        $nextOrder = self::getNextSortOrder($articleSort, $articleOrder, $column);

        return 'index.php?' . http_build_query([
            'action'       => 'monitoring',
            'articleSort'  => $column,
            'articleOrder' => $nextOrder,
      ...
      /**
     * Génère l'URL de tri pour le tableau des commentaires.
     * 
     * @param string $column
     * @param string $articleSort
     * @param string $articleOrder
